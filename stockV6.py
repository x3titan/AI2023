...
def getDay1(stockCode):
    ...
    day1Result = np.zeros(shape=(len(day1) - 10, 4))
    for i in range(0, len(day1Result)):
        day1Result[i][0] = (day1[i+1][3] / day1[i][3] - 1) * 100
        day1Result[i][1] = (day1[i+1][4] / day1[i][2] - 1) * 100
        day1Result[i][2] = (day1[i+1][5] / day1[i][2] - 1) * 100
        day1Result[i][3] = (day1[i+1][2] / day1[i][2] - 1) * 100
    #剔除除权数据
    for i in range(0, len(day1Result)):
        if day1Result[i][0] < -15: day1Result[i][0] = 0
        if day1Result[i][1] < -15: day1Result[i][1] = 0
        if day1Result[i][2] < -15: day1Result[i][2] = 0
        if day1Result[i][3] < -15: day1Result[i][3] = 0
...
def normDay1(stockCode, day1Acc, day1Result):
    #载入归一化数据
    normFilename1 = "./stock/min5/day1Nm" + stockCode + ".dat"
    normFilename2 = "./stock/min5/day1RNm" + stockCode + ".dat"
...
def generateDnn(timeStep):
    ...
    DnnOut, KeepProb = tamPub1.getMultDnn(X, [2440, 4000, 4000, 2500, 1940, 1164, 690, 475, 285, 171, 102, 61, 36, 22, 12, 7, 4], True)      #采用黄金分割方案
    
    #损失函数
    Loss = tf.reduce_sum(tf.square(DnnOut - Y)) * 30
    #Loss = Loss * Loss
    TrainOp = tf.train.AdamOptimizer(1e-5).minimize(Loss)
    ResultAi = DnnOut
    ...
    for batch in range(1 if mode==2 else 1000):
        ...
        if mode==1:
            ...
            if sumLoss/sumCount <= 0.003 and fullTraining:
                ...
                sess.close()
                return 0
            if sumLoss/sumCount < 4 and not fullTraining:
...
def trainStockList():
    #载入股票列表
    stockListTarget = loadTargetStockList(skipCount = 11) #10
