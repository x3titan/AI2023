...
def aiStock3(mode, stockCode, stockName, stockData, stockDataRaw, stockDataAdd, stockDataMul, refStockCode, testOffset = -25, testLength = 50):
    #参数设置
    version = "V3"
    batchSize = 10
    timeStep = 200   #取样天数
    ...
    checkPointFilenameRef = "./stock/save200/SZ300055V1"
    ...
    for i in dataRange:
        singleCount = 0
        if i == dataRange[-1]:
            batchSize = trainEnd - dataRange[-1]
        while True:
            ...
            if singleCount%100 == 0:
                print("======", stockCode + stockName, "======daySn:" + str(i) + "=============")
                print(singleCount, loss, resultAi[0:1,:6])
                print(singleCount, loss, y[0:1,:6])
            ...
            if (loss<0.02): break
            if singleCount>=3000:
                print("!!!训练失败======", stockCode + stockName, "======daySn:" + str(i) + "=============")
                break;
        
        #产生统计数据
        resultAi100 = resultAi * stockDataMul[21:33] - stockDataAdd[21:33]
        resultReal100 = y * stockDataMul[21:33] - stockDataAdd[21:33]
        
        #日志输出
        print("=====批次处理完成，训练达到标准=======", stockCode + stockName, "=========stockSN:" + str(i) + ", loopCount:" + str(singleCount) + ", loss:"  + str(loss), "==========================")
        ...
        if (mode==1 and totalCount > 1000) or (i == dataRange[-1]):
...
#对某只股票进行区间测试
def aiStockTest(stockCode, stockName, stockData, stockDataRaw, stockDataAdd, stockDataMul, testOffset = -25, testLength = 50):
    #参数设置
    version = "V1"
    batchSize = 10
    timeStep = 200   #取样天数
    ...
    try:
        ...
        print("无法获取上次训练保存点:" +  checkPointFilename)
        return False
        
    print("启动测试模式，测试区间：" + str(testBegin+timeStep-1) + "--" + str(str(testEnd+timeStep-1)))
    resultAi100 = np.empty(shape=[0, 12])
    resultReal100 = np.empty(shape=[0, 12])
    resultLoss = np.empty(shape=[0])
    dataRange = range(testBegin, testEnd, batchSize)
    for i in dataRange:
        if i==dataRange[-1]:
            batchSize = testEnd - i
        y = stockData[i+timeStep-1:i+timeStep-1+batchSize, 21:21 + 12]
        ...
        if i == 2787:
            ...
            print(x[0])
        #产生统计数据
        resultAi100 = np.vstack((resultAi100, resultAi * stockDataMul[21:33] - stockDataAdd[21:33])) 
        resultReal100 = np.vstack((resultReal100, y * stockDataMul[21:33] - stockDataAdd[21:33]))
        resultLoss = np.hstack((resultLoss, loss)) 
        
    #日志输出
    print("=====测试完成===" + stockCode + stockName + 
          ", 平均loss:" + str(np.mean(resultLoss)) + 
          ",stockSN:" + str(stockData[dataRange[0] + timeStep - 1][34]) + 
          ", finalBatchSize:" + str(batchSize) + 
          " ==========================")
    print(resultLoss)
    #绘图输出
    show12(
        ...
        resultAi100 = resultAi100, 
        resultReal100 = resultReal100, 
        startSn = stockData[dataRange[0] + timeStep - 1][34])
...
def aiStockSim200(stockCode, stockName, stockData, stockDataRaw, stockDataAdd, stockDataMul):
    ...
    for day200 in range(50):
        ...
        #算一个结果
        batchSize = 10
        i = testBegin + day200 - batchSize + 1
        y = stockData[i+timeStep-1:i+timeStep-1+batchSize, 21:21 + 12]
        x = np.empty(shape=[batchSize,timeStep * 21])
        for j in np.arange(i, i + batchSize):
            x[j-i] = stockData[j:j+timeStep, 0:21].reshape((-1))
        ...
        #收集统计数据
        resultAi100 = np.vstack((resultAi100, resultAi[-1] * stockDataMul[21:33] - stockDataAdd[21:33]))
        resultReal100 = np.vstack((resultReal100, y[-1] * stockDataMul[21:33] - stockDataAdd[21:33]))
        resultDate = np.vstack((resultDate, stockData[i+timeStep-1:i+timeStep-1+batchSize, 33:33 + 1][-1])) 
...
def trainStockList():
    #载入股票列表
    stockListTarget = loadTargetStockList(skipCount = 39)
...
def sim200():
    #载入股票列表
    stockListTarget = loadTargetStockList(skipCount = 46)
    refStockCode = stockListTarget[0][0]
    #循环计算每支股票
    plt.figure(figsize=(16,7))
    for i in range(len(stockListTarget)):
